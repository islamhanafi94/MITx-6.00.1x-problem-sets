...
class PlaintextMessage(Message):
    ...
    def change_shift(self, shift):
        ...
        if shift < 26 and shift >= 0:
            self.shift = shift
            self.encrypting_dict = Message.apply_shift(self, shift)
            self.message_text_encrypted = Message.apply_shift(self, shift)
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        for shift in range(0, 27):
            msgWords = self.apply_shift(shift).split(" ")
            count = 0
            ...
            shiftDict[shift] = count

        maxValue = max(list(shiftDict.values()))
        # result = []
        for key, value in shiftDict.items():
            if maxValue == value:
                result = (key, self.apply_shift(key))
